...
hk_range = [1 , linspace(1.4, 6.5, 10)];       % Stop at 6.5 to avoid kink when going above 8 
% Set of Momentum Thickness Reynolds (Re_theta = rt)
rt_range = logspace(2, 5, 11)*5;               % 500 to 500.000
% Set of Momentum Scaled Plasma Force Field Coefficients (t_theta_p)
t_theta_p_range = logspace(-1, 1, 11);        % 0.1 to 10
...
dataset_file = 'cei_closure_data_export.mat';  % Dataset filename
...
% Make a carriage return with line feed (handy all along)
crlf = char(10); % [char(13),char(10)];

% % Export range vectors (float->symbolic->fortran code)
% Convert to symbolic format
HK_RANGE    = sym(hk_range); 
RT_RANGE    = sym(rt_range);
TPPT_RANGE  = sym(t_theta_p_range);                 % Change variable name for Fortran Code consistency (eg. TPPT2)
% Generate fortran compatible strings for ranges (concatenate a bit later!)
s1 = fortran(HK_RANGE);
s2 = fortran(RT_RANGE);
s3 = fortran(TPPT_RANGE);
% Concatenate strings for all ranges!
s_range  = [s1  , crlf, crlf ...    (hk range)
            s2  , crlf, crlf ...    (rt range)
            s3  , crlf, crlf ]; % (tppt range)

% % Export Range vector Headers
% Make header declaration strings for ranges
s1_h = ['      DOUBLE, DIMENSION(' num2str(length(HK_RANGE  )) ') :: HK_RANGE'];
s2_h = ['      DOUBLE, DIMENSION(' num2str(length(RT_RANGE  )) ') :: RT_RANGE'];
s3_h = ['      DOUBLE, DIMENSION(' num2str(length(TPPT_RANGE)) ') :: TPPT_RANGE'];
% Concatenate strings for all range headers!
s_range_headers = [s1_h, crlf, s2_h, crlf, s3_h, crlf];

% Export Grid (float->symbolic->fortran code)
% Convert cei_grid to symbolic format
cei_grid_sym = sym(cei_grid);
% Export from symbolic format to Fortran code (file output)
fortran(cei_grid_sym,'file','4_fortan_output/CEI_GRID.f90')


fid = fopen('4_fortan_output/CEI_HEADERS.f90','w');
if fid>=0
    fprintf(fid, '%s\n', s_ranges)
    fclose(fid)
end
