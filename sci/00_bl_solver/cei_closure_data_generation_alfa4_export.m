...
hk_range = [1 , linspace(1.4, 6.5, np-1)];       % Stop at 6.5 to avoid kink when going above 8 
% Set of Momentum Thickness Reynolds (Re_theta = rt)
rt_range = logspace(2, 5, np)*5;               % 500 to 500.000
% Set of Momentum Scaled Plasma Force Field Coefficients (t_theta_p)
t_theta_p_range = [0 , logspace(-1, 1, np-1)];        % 0.1 to 10
...
% % Start by makin a carriage return with line feed (handy all along)
...
% % Make CEI grid strings (float->symbolic->fortran string)
CEI_TABLE = sym(cei_grid);   % Convert cei_grid to symbolic format
...
fid = fopen('4_fortan_output/CEI_DATA.f90','w');
if fid>=0
    fprintf(fid, '%s\n', s_headers);
    fprintf(fid, '%s\n', s_arrays);
    fclose(fid)
