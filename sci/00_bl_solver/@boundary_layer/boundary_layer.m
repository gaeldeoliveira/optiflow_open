classdef boundary_layer < handle
...
%         function [rhs_theta, rhs_hs, rhs_ctau] = canonical_rhs(BL, t, dstr, ctau, x)
%             % Typical Test Values
%             % t       = 0.002;
%             % dstr    = 0.003;
...
%             rhs_hs = (hs ./ t) .* di - (hs ./ t) .* (cf / 2) - (2 * hc ./ hs + 1 - h) .* (hs ./ ue) .* due_dxi;
%             
...
%            [ h, h_dstr, h_t] = H(dstr , t);                           % Looks ok
            ...
            % Now calculate Cf
            [ cf, cf_hk, cf_rt, cf_msq  ] = cft( hk, rt, msq);       % Looks ok
...
%            rhs_ctau = (ctau.^2 ./ d) .* scc .* (ctz - ctau);
        ...
        function [rhs_theta, rhs_dstr, rhs_ctau] = transformed_rhs(BL, t, dstr, ctau, x)
            % % Get canonical rhs
            ...
            dbar_theta  = h - ue_over_nue .* hs_rt ./ den;
            dbar_hs     = 1 ./ den;
            
            dbar_ue     = - (t ./ ue) .* ue_over_nue .* hs_rt ./ den;
            dbar_msq    = - hs_msq ./ den;
            
            % And transform!         
            rhs_dstr = dbar_theta .* rhs_theta + ...
                          dbar_hs .* rhs_hs    + ...
                          dbar_ue .* due_dxi   + ...
                         dbar_msq .* dmsq_dxi;
        end
        
        function [rhs_theta, rhs_dstr, rhs_ctau] = transformed_rhs2(BL, t, dstr, ctau, x)
            % % Get canonical rhs
            [rhs_theta, rhs_hs, rhs_ctau] = canonical_rhs(BL, t, dstr, ctau, x);
