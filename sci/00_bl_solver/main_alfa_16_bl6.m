...
CEI.cei_softening = cei_softening;
...
BL_cf  = boundary_layer6_plasma_cf_mod();

% Set Hooks between BL object and and PD and CEI objects
BL_on.PD_cell   = PD_cell   ; BL_cf.PD_cell   = PD_cell   ;
BL_on.CEI  = CEI            ; BL_cf.CEI  = CEI            ;

% Set nue 
BL_on.u_inf_over_nu_inf  = EDL_on.u_inf_over_nu_inf;
BL_off.u_inf_over_nu_inf = EDL_off.u_inf_over_nu_inf;
BL_cf.u_inf_over_nu_inf  = EDL_on.u_inf_over_nu_inf;
...
BL_cf.set_forcing_terms(EDL_on.xi_vector, EDL_on.ue_vector, msq_vector_on);
...
% % Average Initial Conditions to reduce starting point uncertainty
% and improve comparison consistencty
x0 = 0.5*(x0_on + x0_off);
t0 = 0.5*(t0_on + t0_off);
h0 = 0.5*(h0_on + h0_off);

% Set and guess ctau0 from equilibrium value
BL_on.set_equilibrium_initial_conditions(t0, h0);          % Initial Shear Stress Coefficient determined as if BL was in equilibrium (which more less never true!!!)
BL_cf.set_equilibrium_initial_conditions(t0, h0);          % Initial Shear Stress Coefficient determined as if BL was in equilibrium (which more less never true!!!)
...
BL_on.integrate_ODE( x0, xend);
BL_cf.integrate_ODE( x0, xend);

%% Plot Results
% Extract some results
x_sol_on       = BL_on.sol.x      ; x_sol_off      = BL_off.sol.x      ; x_sol_cf       = BL_cf.sol.x      ;
t_sol_on       = BL_on.sol.y(1,:) ; t_sol_off      = BL_off.sol.y(1,:) ; t_sol_cf       = BL_cf.sol.y(1,:) ;
h_sol_on       = BL_on.sol.y(2,:) ; h_sol_off      = BL_off.sol.y(2,:) ; h_sol_cf       = BL_cf.sol.y(2,:) ;
ctau_sol_on    = BL_on.sol.y(3,:) ; ctau_sol_off   = BL_off.sol.y(3,:) ; ctau_sol_cf    = BL_cf.sol.y(3,:) ;
...
dstr_sol_cf    = h_sol_cf  .* t_sol_cf;
% Plot the Shape Factor Evolution
figure(1)
subplot(2,1,1)
plot(x_sol_on , dstr_sol_on  , ...
     x_sol_cf, dstr_sol_cf , ...
     EDL_on.xi_vector , EDL_on.dstr_vector , ...
     x_sol_off, dstr_sol_off , ...
     EDL_off.xi_vector, EDL_off.dstr_vector);
legend('Plasma ON  - Computed', ...
       'Plasma ON  - Computed (Mod CF)' , ...
       'Plasma ON  - Experimental' , ...
       'Plasma OFF - Computed', 'Plasma OFF - Experimental' , ...
       'Location', 'Best');
   
xlabel('x') ; ylabel('delta* - Displacement Thickness (adim.)') ; grid on
title(['Comparison with Experiment -- U_{inf}=' , U_str ,  'm/s  AOA=' , alpha_str , 'deg  Location=', act_str] )

% Plot the Momentum Thickness Evolution
subplot(2,1,2)
plot(x_sol_on , t_sol_on  , ...
     x_sol_cf, t_sol_cf , ...
     EDL_on.xi_vector , EDL_on.theta_vector , ...
     x_sol_off, t_sol_off , ...
     EDL_off.xi_vector, EDL_off.theta_vector);
xlabel('x') ; ylabel('theta - Momentum Thickness (adim.)') ; grid on

% Save to file
savefig(['BL1_',act_str,'_a',alpha_str,'_U',U_str,'.fig']);


figure(2)
subplot(2,1,1)
plot(x_sol_on , h_sol_on  , ...
     x_sol_cf , h_sol_cf , ...
     EDL_on.xi_vector , EDL_on.dstr_vector  ./ EDL_on.theta_vector , ...
     x_sol_off, h_sol_off , EDL_off.xi_vector, ...
     EDL_off.dstr_vector ./ EDL_off.theta_vector);
...
% Save to file
savefig(['BL2_',act_str,'_a',alpha_str,'_U',U_str,'.fig']);

%% Write Comparison Strings
delta_t_on      = t_sol_on(end) - t_sol_off(end);
delta_t_cf      = t_sol_cf(end) - t_sol_off(end);
delta_t_ex      = EDL_on.theta_vector(end) - EDL_off.theta_vector(end);

delta_dstr_on   = dstr_sol_on(end) - dstr_sol_off(end);
delta_dstr_cf   = dstr_sol_cf(end) - dstr_sol_off(end);
delta_dstr_ex   = EDL_on.dstr_vector(end) - EDL_off.dstr_vector(end);


disp(['Case              |      dstr       |        theta        |'])
disp(['Computed          |    ' num2str(delta_dstr_on) , '   |      ' , num2str(delta_t_on) , '     |'])
disp(['Computed (mod Cf) |    ' num2str(delta_dstr_cf) , '   |      ' , num2str(delta_t_cf) , '     |'])
disp(['Experimental      |    ' num2str(delta_dstr_ex) , '   |      ' , num2str(delta_t_ex) , '     |'])
