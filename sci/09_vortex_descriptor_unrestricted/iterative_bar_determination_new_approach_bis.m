...
[ dstr_bar_ref,  dstr_tilde_ref,  dstr_hat_ref] = SMI.dstr_direct( state.hk, state.rt, state.ue, nu_inf, state.u_tilde);
[theta_bar_ref, theta_tilde_ref, theta_hat_ref] = SMI.theta_direct(state.hk, state.rt, state.ue, nu_inf, state.u_tilde);
% Display reference values
disp(['Starting Reference   n = ' num2str( 0)]);
disp(['  dstr_bar_ref = ' num2str(  dstr_bar_ref)  '      dstr_hat_ref = ' num2str(  dstr_hat_ref)]);
disp([' theta_bar_ref = ' num2str( theta_bar_ref)  '     theta_hat_ref = ' num2str( theta_hat_ref)]);

% Start with initial guesses (make as first current)
dstr_bar_i  = dstr_hat_ref;
theta_bar_i = theta_hat_ref;
...
[ ~,  ~,  dstr_hat_i] = SMI.dstr_direct( hk_bar_i , rt_bar_i , state.ue, nu_inf, state.u_tilde);
[ ~,  ~, theta_hat_i] = SMI.theta_direct(hk_bar_i , rt_bar_i , state.ue, nu_inf, state.u_tilde);

% Display Values
disp(['  dstr_bar_i   = ' num2str(  dstr_bar_i  )  '      dstr_hat_i   = ' num2str(  dstr_hat_i  )]);
disp([' theta_bar_i   = ' num2str( theta_bar_i  )  '     theta_hat_i   = ' num2str( theta_hat_i  )]);
...
% Make new guess
dstr_bar_i  =  dstr_bar_i - 0.01 * delta_dstr_hat ;
theta_bar_i = theta_bar_i - 0.01 * delta_theta_hat ;
...
% Plot
plot(n, dstr_bar_i    , 'ok'); hold on;
plot(n, dstr_bar_ref  , 'or'); hold on;
plot(n, theta_bar_i   , 'xk'); grid on;
plot(n, theta_bar_ref , 'xr'); grid on;
