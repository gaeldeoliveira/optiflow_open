...
x_end     =    5* h_VG;%0.25;%;0.5;        % End Time (0.25 = 50 * h_vg)
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    if (n/ information_period == round(n / information_period))        
        ...
        grid on; axis([-0.2 1.2 0 2]);
        % title('Central Symmetry Line');
        title(['x/h = ' num2str(round(state.x / h_VG*100)/100), 'and z/S = ', num2str(0)])
        ...
        title(['x/h = ' num2str(round(state.x / h_VG*100)/100), 'and z/S = ', num2str(-1)])
        xlabel('u/u_{inf}'); ylabel('y/\delta');
        ...
        title(['x/h = ' num2str(round(state.x / h_VG*100)/100), 'and z/S = ', num2str(0.5)])
        xlabel('u/u_{inf}'); ylabel('y/\delta');
        % Output to File
        if img_output == true
            set(gcf , 'PaperType', 'A5');
