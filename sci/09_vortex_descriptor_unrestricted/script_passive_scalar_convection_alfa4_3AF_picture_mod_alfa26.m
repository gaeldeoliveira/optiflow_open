...
hk0       = 1.4;                      % [adim.] Shape Factor
rt0       = 3000;                     % [adim.] Reynolds Theta of Starting Flow
delta0    = 25e-3; % ...
...
D_u_tilde = 0.010;                    % Artificial Viscosity on C_tilde Field (0.002 seems nice for 100*100)
x0        = 0;                        % Starting Time
x_end     = 2.0;%;0.5;                      % End Time
dx        = 0.0000125;                % TimeStep Weird stuff happens for dt = 0.1 (time step constraint?)
dx_info   = 0.0125000;                % Time between information events
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    if (n/ information_period == round(n / information_period))        
        ...
        surf(z_mesh/S, y_mesh/delta0, state.u_tilde / state.ue)
        xlabel('z/S'); ylabel('y/\delta'); colorbar;
        title(['u^{tilde} / u_e    at x = ' h_str]);
        view(2); shading flat;  caxis([-0.15 0.15]);
        axis([min(CM.z_range)/S max(CM.z_range)/S min(CM.y_range)/delta0 max(CM.y_range)/delta0]);
        % Output to File
        ...
        surf(z_mesh/S, y_mesh/delta0, state.u_tilde / state.ue)
        xlabel('z/S'); ylabel('y/\delta'); colorbar;
        title(['u_{x}^{tilde} / u_e    at x = ' h_str]);
        view(2); shading flat; caxis([-0.15 0.15]);
        axis([min(CM.z_range)/S max(CM.z_range)/S min(CM.y_range)/delta0 max(CM.y_range)/delta0]);               
        ...
        xlabel('z/S'); ylabel('y/\delta'); colorbar;
        title(['(u^{tilde}+u^{bar}) / u_e    at x = ' h_str]);
        ...
        grid on; axis([-0.5 1.5 0 max(CM.y_range)/delta0]);
        ...
        xlabel('u/u_{inf}'); ylabel('y/\delta');
        legend('Mixed Flow' , 'Pure Shear Flow', 'Total Flow')
        % Output to File
        set(gcf , 'PaperType', 'A5');
        orient landscape;
        ...
        subfigure(122)
        plot3(  state.z_v/S    , state.y_v/delta0, 'ok'); hold on; grid on;
        plot3( -state.z_v/S    , state.y_v/delta0, 'or');
        plot3(  state.z_v/S - 2, state.y_v/delta0, 'ok');
        plot3( -state.z_v/S + 2, state.y_v/delta0, 'or');       
        axis([min(CM.z_range)/S max(CM.z_range)/S min(CM.y_range)/delta0 max(CM.y_range)/delta0])
        ...
        % Shape Factor
        subplot(211)
        plot( state.x , h_bar   , 'ok'); hold on; grid on;
        plot( state.x , h_hat   , 'or');
        xlabel('x/L'); ylabel('H_k - Shape factor')
        title('Shape Factor')
        legend('Pure Shear Flow' , ' Total Flow')
        axis([x0-eps state.x 1.3 1.5]);
        % Displacement Thickness
        subplot(212)
        plot( state.x , dstr_bar   , 'ok'); hold on; grid on;
        plot( state.x , dstr_hat   , 'or');
        plot( state.x , dstr_tilde , 'ob');
        xlabel('x/L'); ylabel('\delta* - Displacement Thickness')
        ...
        subplot(211)
        plot( state.x , state.sigma_v , 'ok'); hold on; grid on;
        xlabel('x/L'); ylabel('\omega^{max}_{0}')
        title('Peak Vorticity in Lamb Vortex Core')
        axis([x0-eps state.x 0 state0.sigma_v*1.2]);
        % Circulation
        subplot(212)
        plot( state.x , state.gamma_v , 'ok'); hold on; grid on;
        xlabel('x/L'); ylabel('\Gamma_{v}')
        title('Filament Circulation per Unit Lenght')
        axis([x0-eps state.x 0 state0.gamma_v*1.2]);
        % Output to File
        set(gcf , 'PaperType', 'A5');
        orient landscape;
