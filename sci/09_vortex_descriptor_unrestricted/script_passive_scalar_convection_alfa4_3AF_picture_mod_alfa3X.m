...
%
% TEMPORARY CHANGE
% Original: dsigma_v_dt_v = - (1 / (4*pi())) * (pod_state.gamma_v ./ nu_v) * (1 ./ pod_state.t_v).^2;
% Current : dsigma_v_dt_v = - (1 / (4*pi())) * (pod_state.gamma_v ./ D_u_tilde) * (1 ./ pod_state.t_v).^2;
...
x0        = 0;                        % Starting Time
x_end     = 0.125;%0.25;%;0.5;               % End Time
dx        = 0.0000125;                % TimeStep Weird stuff happens for dt = 0.1 (time step constraint?)
dx_info   = 0.0025000;                % Time between information events
...
for n = 0:round(x_end/dx)
    ...
    dsigma_v_dt_v = - (1 / (4*pi())) * (state.gamma_v ./ D_u_tilde) * (1 ./ state.t_v).^2;
    % Determine convection speed at (primary) vortex core center
    u_bar_v     = SF.u_bar_at_y(state.hk, state.rt, state.ue, nu_inf, state.y_v);
    % Determine lagragiant time derivative to space (x) at core center
    dT_v_dx = 1 / u_bar_v;        % don't confuse with dT_dx of u_tilde field! 
...
for n = 0:round(x_end/pod_dx)
    ...
    dsigma_v_dt_v = - (1 / (4*pi())) * (pod_state.gamma_v ./ D_u_tilde) * (1 ./ pod_state.t_v).^2;
    % Determine convection speed at (primary) vortex core center
    u_bar_v     = SF.u_bar_at_y(pod_state.hk, pod_state.rt, pod_state.ue, nu_inf, pod_state.y_v);
    % Determine lagragiant time derivative to space (x) at core center
    dT_v_dx = 1 / u_bar_v;        % don't confuse with dT_dx of u_tilde field! 
