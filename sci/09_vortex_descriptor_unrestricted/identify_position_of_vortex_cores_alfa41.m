...
% % Header copied from:
%       sct_passive_scalar_convection_alfa4_3AF_picture_mod_alfa41ZPG
...
state_05h = load('20180812T004926/matfiles/full_state_asy_5h.mat' ); y_num(1) = state_05h.state.y_v; z_num(1) = state_05h.state.z_v;
state_06h = load('20180812T004926/matfiles/full_state_asy_6h.mat' ); y_num(2) = state_06h.state.y_v; z_num(2) = state_06h.state.z_v;
state_07h = load('20180812T004926/matfiles/full_state_asy_7h.mat' ); y_num(3) = state_07h.state.y_v; z_num(3) = state_07h.state.z_v;
state_08h = load('20180812T004926/matfiles/full_state_asy_8h.mat' ); y_num(4) = state_08h.state.y_v; z_num(4) = state_08h.state.z_v;
state_09h = load('20180812T004926/matfiles/full_state_asy_9h.mat' ); y_num(5) = state_09h.state.y_v; z_num(5) = state_09h.state.z_v;
state_10h = load('20180812T004926/matfiles/full_state_asy_10h.mat'); y_num(6) = state_10h.state.y_v; z_num(6) = state_10h.state.z_v;
% And (semi)automatically
y_num_full = zeros(1,49);
z_num_full = zeros(1,49);
for n_num=1:length(y_num_full)
   state_tmp = load(['20180812T004926/matfiles/full_state_asy_', num2str(n_num), 'h.mat' ]); 
   y_num_full(n_num) = state_tmp.state.y_v; 
   z_num_full(n_num) = state_tmp.state.z_v; 
end
state_tmp = load(['20180812T004926/matfiles/full_state_asy_', '-0.5', 'h.mat' ]); 
...
% Plot left side
h0 = plot(z_lB(d_listB_subset) / S, y_luB(d_listB_subset) / delta0, 'o-', 'Color', [0 , 147,  208]/255); hold on;
h1 = plot(-z_num_full / S, y_num_full /delta0 , 'k.:'); 
plot(  1     * (d_VG / 2) / S,    1  * h_VG /delta0, 'k-x')
plot( [1, 1] * (d_VG / 2) / S, [0 1] * h_VG /delta0, 'k-')

% Plot right side
h2 = plot(z_rB(d_listB_subset) / S, y_ruB(d_listB_subset) / delta0, 'o-', 'Color', [0 , 147,  208]/255);
h3 = plot( z_num_full / S, y_num_full /delta0 , 'k.:'); grid on;
plot(- 1     * (d_VG / 2) / S,    1  * h_VG /delta0, 'k-x')
plot(-[1, 1] * (d_VG / 2) / S, [0 1] * h_VG /delta0, 'k-')
...
subplot(223)
errorbar(d_list, z_mean/S, 2 * mean(std_z) * ones(size(std_z)) /S , 'Color', [0 , 147,  208]/255); hold on
plot(d_list, z_num / S, 'k.:'); grid on;
...
subplot(224)
errorbar(d_list, y_mean/delta0, 2 * mean(std_y) * ones(size(std_y)) /delta0, 'Color', [0 , 147,  208]/255); hold on
plot(d_list, y_num / S, 'k.:'); grid on;
...
print('-depsc', ['vortex_core_positions_ZPG_extrapolated.eps']) %#ok<NBRAK>
savefig vortex_core_positions_ZPG_extrapolated.fig


%% Now compare induction of vortex system along a vertical line
% Set right induction function!
VD.induction_function = 4;
...
[w_mesh, ~, ~] = VD.induction_on_mesh(CM, state.z_v, state.y_v, state.gamma_v, state.sigma_v);
...
h_06n = plot(y_line, w_line_num06/state.ue);
h_08n = plot(y_line, w_line_num08/state.ue);
h_10n = plot(y_line, w_line_num10/state.ue);
...
figure(406)
subplot(221)
h_06  = plot(y_line, w_line06      /state.ue, '-', 'Color', [0 , 147,  208]/255); hold on
h_06n = plot(y_line, w_line_num06 /state.ue , 'k:'); grid on
xlabel('y/\delta_{ref}'); ylabel('w/U_e')
% legend(['Experimental (at z_{core}= ' , num2str(z_l(2 )/S, 2), 'S )'], ...
%        ['Numerical    (at z_{core}= ' , num2str(-state_06h.state.z_v/S, 2), ' S )'], ...
%        'Location', 'SouthEast')
legend('Experiment', 'Prediction', 'Location', 'SouthEast')
title('Spanwise Induction at x = 6h')
axis([0 1.4 -0.3 0.3])

subplot(222)
h_10  = plot(y_line, w_line10      /state.ue, '-' , 'Color', [0 , 147,  208]/255); hold on
h_10n = plot(y_line, w_line_num10  /state.ue, 'k:'); grid on
xlabel('y/\delta_{ref}'); ylabel('w/U_e')
title('Spanwise Induction at x = 10h')
% legend(['Experimental (at z_{core}= ' , num2str(z_l(6 )/S, 2), 'S )'], ...
%        ['Numerical    (at z_{core}= ' , num2str(-state_10h.state.z_v/S, 2), ' S )'], ...
%        'Location', 'SouthEast')
legend('Experiment', 'Prediction', 'Location', 'SouthEast')
axis([0 1.4 -0.3 0.3])
...
set(gcf , 'PaperType', 'A5');
orient landscape;
print ('-dpdf', ['spanwise_induction_ZPG_extrapolated.pdf']) %#ok<NBRAK>
print ('-depsc', ['spanwise_induction_ZPG_extrapolated.eps']) %#ok<NBRAK>
savefig spanwise_induction_ZPG_extrapolated.fig
