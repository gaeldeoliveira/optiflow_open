...
x0        = - (3/4) * h_VG;           % Starting Time (0, or -h_vg/4 , or /3/4*h_vg to enable predevelopment, vortex does not start at TE of Vg)
x_end     =    0.25 + h_VG;%0.25;%;0.5;        % End Time (0.25 = 50 * h_vg)
...
% Set new step size
pod_dx                 = 1.25e-2;      % Up to 1000 times bigger than the dx of the normal stuff!
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    if (n/ pod_information_period == round(n / pod_information_period))        
        ...
        % % % Plotting and File Output
        % % Update Output Identifier String        
        id_str = ['_pod_' num2str(round(pod_state.x / h_VG*10)/10) 'h'];
