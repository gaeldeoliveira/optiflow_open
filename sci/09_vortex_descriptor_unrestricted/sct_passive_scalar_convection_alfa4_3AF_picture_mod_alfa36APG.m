...
%   x = 10h
%   x = 15h
%   x = 25h (and eventually 50h...)
% FOR POD SHOW:
%   Make sure time step is small enough
%   Keeping only two POD 
%TEMPORARY:
%Regular      : ue_channel = ue_ref * hd_ref ./  hd_channel;
%Current trial: ue_channel = ue_ref * hd_ref ./ (hd_channel + (hd_channel - hd_ref));
% (accentuated edge velocity distribution - double gradient! there seems to
% be something inconsistent in the description of the wall!) 
...
% Position of slats at which Height was measured (tunnel reference frame) [mm-to->m]
xd_channel = [50 , 150, 250, 350, 450, 550, 650, 750, 850, 950, 1050, 1150, 1250, 1350, 1450, 1550, 1650, 1750, 1850, 1950] * 0.001; 
% Height of tunnel top wall slats (tunnel reference frame) [mm-to->m]
hd_channel = [258, 258, 258, 258, 258, 260, 263, 265, 266, 268,  269,  270,  271,  272,  272,  272,  270,  268,  265,  264] * 0.001;
% Streamwise position of vane trailing edge (tunnel reference frame) [mm-to->m]
xd_VG_TE   =  985 * 0.001;
% Streamwise position used for speed reference (tunnel reference frame) [mm-to->m] (the first PIV snapshot (at xTE+5hVG) is unreliable, used the second one (at xTE+6hVG)!
xd_ref     =  xd_VG_TE + 6*h_VG;
% Reference edge speed at streamwise position used for speed reference
ue_ref     = 14.73;
% Find height of tunnel at reference position (xd_ref) [m]
hd_ref     = interp1(xd_channel, hd_channel, xd_ref);
% Compute edge speed distribution of channel [m/s]
%ue_channel = ue_ref * hd_ref ./ hd_channel;
%ue_channel = ue_ref * hd_ref ./ (hd_channel +   (hd_channel - hd_ref));
ue_channel = ue_ref * hd_ref ./ (hd_channel + 3*(hd_channel - hd_ref));
% Make translated streamwise reference frame of channel
xi_channel = xd_channel - xd_ref;

% Set Outer Flow Parameters
rho_inf     = 1.225;                  % [kg/m3]   Free Stream Density
nu_inf      = 1.461e-5;               % [m2 s-1]  Kinematic Viscosity (nu = mu / rho)
u_inf       = 14.73;%15.16(former,ZPG)% [m/s]     Reference Edge Free Stream Speed (now for APG)
...
hk0       = 1.40;%1.3567; %1.35(former,ZPG)% [adim.] Shape Factor (now for APG)
rt0       = 2851.6; %2466(former,ZPG)% [adim.] Reynolds Theta of Starting Flow (now for APG)
delta0    = ... % 25e-3; 
 delta_99(hk0,rt0,u_inf_over_nu_inf); % [m    ] Boundary Layer Thickness (0.0160 for ZPG, 0.0187 for APG)
...
x0        = 0;                        % Starting Time
x_end     = 0.25;%0.25;%;0.5;         % End Time
dx        = 0.0000250;                % TimeStep Weird stuff happens for dt = 0.1 (time step constraint?)
dx_info   = 0.0050000;                % Time between information events
...
ue_vector  = interp1(xi_channel, ue_channel, xi_vector);   % Reinterpolated edge velocity distribution
...
% Set viscosity/reynolds scale for boundary layer solver (transforms to local ue_over_nue inside solver)
BL.u_inf_over_nu_inf = u_inf_over_nu_inf; 

% Set forcing terms (order matters! must be done after u_inf_over_nu_inf was set)
% (normally we work in adimensional terms, as described in object, but here 
%  we are working with everything dimensional with should also work as long 
%  as are consistent for u_inf_over_nu_inf, xi_vector and ue_vector)
...
state.y_v     = h_VG;             % [m /s ] distance of filament core center to wall
...
snapshots = struct();
snapshots.N_steps      = round(x_end/dx)+1;
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    if (n/ information_period == round(n / information_period))        
        ...
        plot( new_state.u_tilde(:, j_center )/ new_state.ue, CM.y_range/delta0, ...
                          ...
                          ED.interp_velocities(state.x / h_VG, CM.y_range/ delta0, 0) / u_inf, CM.y_range/delta0 + 0.02, 'k');
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    if (n/ pod_information_period == round(n / pod_information_period))        
        ...
        % % % Plotting and File Output
        % % Update Output Identifier String        
        id_str = ['_' num2str(round(pod_state.x / h_VG*10)/10) 'h'];
        h_str  = [num2str(round(pod_state.x / h_VG*10)/10) ' h^{VG}'];
        ...
        title('Shape Factor')
        legend('Pure Shear Flow', 'Total Flow')
        axis([x0-eps pod_state.x/h_VG 1.3 1.5]);
        % Displacement Thickness
        subplot(212)
        ...
        title('Displacement Thickness')
        legend('Pure Shear Flow' , ' Total Flow', 'Mixed Flow')
        axis([x0-eps pod_state.x 0 2*(hk0*rt0/(u_inf/nu_inf))]);
        % Output to File
        if img_output == true
            set(gcf , 'PaperType', 'A5');
            orient landscape;
        ...
        xlabel('x/h^{VG}'); ylabel('\omega^{max}_{0}')
        title('Peak Vorticity in Lamb Vortex Core')
        axis([x0-eps pod_state.x/h_VG 0 state0.sigma_v*1.2]);
        % Circulation
        subplot(212)
        plot( pod_state.x / h_VG, pod_state.gamma_v , 'ok'); hold on; grid on;
        xlabel('x/h^{VG}'); ylabel('\Gamma_{v}')
        title('Filament Circulation per Unit Lenght')
        axis([x0-eps pod_state.x/h_VG 0 state0.gamma_v*1.2]);
        % Output to File
        if img_output == true
            set(gcf , 'PaperType', 'A5');
            orient landscape;
