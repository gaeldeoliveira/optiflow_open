...
%% Now look at velocity profiles to redo initial conditions estimation
...
%% And get offsets at these points
offset_l = interp1(ED_bf.data_struct_cell{1}.z(1,:) / S, ED_bf.data_struct_cell{1}.y_offset_extrapolation / delta_ref, -1.0);
offset_c = interp1(ED_bf.data_struct_cell{1}.z(1,:) / S, ED_bf.data_struct_cell{1}.y_offset_extrapolation / delta_ref,  0.0);
offset_r = interp1(ED_bf.data_struct_cell{1}.z(1,:) / S, ED_bf.data_struct_cell{1}.y_offset_extrapolation / delta_ref,  1.0);
%% And get 
...
% Now make skin friction and u_tau for these conditions (mach 0)
cft_0   = cft( Hk_0, rt_0, 0);
u_tau_0 = u_inf * sqrt(cft_0/2);
y_plus_c= u_tau_0 * u_target_y_c / nu_inf;

% 
% %% Make initial conditions plot
% figure(1)
% subplot(211)
...
% set(gcf , 'PaperType', 'A5');
% orient landscape;
% print ('-dpdf', ['swafford_velocity_profile_ZPG_extrapolated_levenberg.pdf']) %#ok<NBRAK>
...
% Reinterpolate experimental velocities to make everything nicer
target_y_c_HD = linspace(0, 2,length(target_y_c)*10);
u_target_y_l  = interp1(target_y_l, u_target_y_l, target_y_c_HD );
u_target_y_c  = interp1(target_y_c, u_target_y_c, target_y_c_HD );
u_target_y_r  = interp1(target_y_r, u_target_y_r, target_y_c_HD );
% Overload
target_y_l    = target_y_c_HD;
target_y_c    = target_y_c_HD;
target_y_r    = target_y_c_HD;
% Regenerate Swafford
SP.update_hk_rt_pair(Hk_0, rt_0);
u_target_y_c_swafford = SP.evaluate_profile(target_y_c_HD * delta_ref/ theta_c);

% Make validity indices
i_valid_l = target_y_l > offset_l;
i_valid_c = target_y_c > offset_c;
i_valid_r = target_y_r > offset_r;


subplot(221) 
exp_l = plot(u_target_y_l(i_valid_l),          target_y_l(i_valid_l), '-' , 'Color', [0  , 147, 208]/255); hold on; grid on;
exp_c = plot(u_target_y_c(i_valid_c),          target_y_c(i_valid_c), '-' , 'Color', [252, 170,  13]/255); 
exp_r = plot(u_target_y_r(i_valid_r),          target_y_r(i_valid_r), '-' , 'Color', [252 , 38,  13]/255); 
        plot(u_target_y_l(i_valid_l),          target_y_l(i_valid_l), '-' , 'Color', [0  , 147, 208]/255); 
swaf  = plot(u_target_y_c_swafford, target_y_c, 'k:'  ); 
axis([0 1.05 0 1.4]);
xlabel('U/U_e'); ylabel('y/\delta_{ref}');
% legend([exp_l , exp_c, exp_r, swaf],  'Experimental at z/S=-1', 'Experimental at z/S=0', 'Experimental at z/S=1' , 'Swafford reconstruction', 'Location', 'NorthWest')
lg1 = legend([exp_l , exp_c],  'Experimental at z/S=-1', 'Experimental at z/S=0', 'Location', 'South');
title('Velocity Profile - Linear Scale')

subplot(222)
exp_l = semilogy(u_target_y_l(i_valid_l) * u_inf /u_tau_0,          u_tau_0 * target_y_l(i_valid_l) * delta_ref / nu_inf, '-', 'Color', [0 , 147,  208]/255 ); hold on; grid on;
exp_c = semilogy(u_target_y_c(i_valid_c) * u_inf /u_tau_0,          u_tau_0 * target_y_c(i_valid_c) * delta_ref / nu_inf, '-', 'Color', [252, 170,  13]/255 ); 
exp_r = semilogy(u_target_y_r(i_valid_r) * u_inf /u_tau_0,          u_tau_0 * target_y_r(i_valid_r) * delta_ref / nu_inf, '-', 'Color', [252 , 38,  13]/255 ); 
        semilogy(u_target_y_l(i_valid_l) * u_inf /u_tau_0,          u_tau_0 * target_y_l(i_valid_l) * delta_ref / nu_inf, '-', 'Color', [0 , 147,  208]/255 ); 
swaf  = semilogy(u_target_y_c_swafford   * u_inf /u_tau_0,          u_tau_0 * target_y_c * delta_ref / nu_inf, 'k:'); 
axis([0, 1.05*u_inf/u_tau_0, 1, u_tau_0*1.4*delta_ref/nu_inf])
%axis([0 1.05 min(target_y_l) max(target_y_l)]);
xlabel('U/U_{\tau}'); ylabel('ln(y^+)');
% legend([exp_l , swaf],  'Experimental Baseflow', ['Swafford (H_k =' num2str(Hk_0,3) '  Re_{\theta} =' num2str(rt_0,4) ])
%legend([exp_l , exp_c, exp_r, swaf],  'Experimental at z/S=-1', 'Experimental at z/S=0', 'Experimental at z/S=1' , 'Swafford reconstruction', 'Location', 'NorthWest')
lg2 = legend([exp_r, swaf],  'Experimental at z/S=1' , 'Swafford reconstruction', 'Location', 'South');
title('Velocity Profile - Log Scale')

lg1.Position = [0.1741 0.4238 0.2393 0.0631];
lg2.Position = [0.6125 0.4238 0.2464 0.0631];

set(gcf , 'PaperType', 'A5');
orient landscape;
print ('-dpdf', ['initial_conditions_overview_ZPG_extrapolated_levenberg.pdf']) %#ok<NBRAK>
print ('-depsc', ['initial_conditions_overview_ZPG_extrapolated_levenberg.eps']) %#ok<NBRAK>
savefig initial_conditions_overview_ZPG_extrapolated_levenberg.fig
