...
pod_snapshots.N_steps      = round(x_end/dx)+1;
...
for n = 0:round(x_end/dx)
    ...
    % March parameters (primary BL)
    new_pod_state.theta     = pod_state.theta + d_theta_dx * dx;
    new_pod_state.hk        = pod_state.hk    + d_h_dx     * dx;
    new_pod_state.ctau      = pod_state.ctau  + d_ctau_dx  * dx;
    ...
    dz_v_dx       = w_v * dT_v_dx; 
    % March filament position in space
    new_pod_state.y_v = pod_state.y_v + dy_v_dx * dx;
    new_pod_state.z_v = pod_state.z_v + dz_v_dx * dx;
    ...
    dsigma_v_dx = dsigma_v_dt_v * dT_v_dx;
    % March Core Diffusion Equations
    new_pod_state.t_v     = pod_state.t_v     + dT_v_dx     * dx;
    new_pod_state.sigma_v = pod_state.sigma_v + dsigma_v_dx * dx;
...
%     sigma_v = sigma_v + dsigma_v_dx * dx
    ...
    new_pod_state.a1 = pod_state.a1 + da1_dx  * dx;
    new_pod_state.a2 = pod_state.a2 + da2_dx  * dx;
    new_pod_state.a3 = pod_state.a3 + da3_dx  * dx;
    ...
    % % March stuff, Transmit static States, Make it Consistent
    % Position
    new_pod_state.x       = pod_state.x + dx;
