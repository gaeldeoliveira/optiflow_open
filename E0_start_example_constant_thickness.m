...
%
%       File: 
%           Basic Multiobjective Optimization Example Case with prescribed
%           thickness.
%           Constant thickness constraint is implemented implicitly by
%           recasting design space using the shape_dynamizer (SDD) object.
%
%           Aerodynamic Goal: L/D over a range of Cls
%           Structural  Goal: Maximum Airfoil Thickness (meaningless, must
%           be replace with some other aerodynamic/structural goal for
%           practical cases).
%
...
%% Create Shape and Parametrization handling objects
 ...
 N_dummy_parameters = 1;                                                        % Number of parameters that are not shape definition ones (0 = thin T.E. airfoils, 1 = thick T.E. airfoils , 2+ additional parameters)
...
%% Set Thickness Projection Constraints
CM.ref_pop_array_bounds(:,p_upper.order);
mean_TE_wedge = CM.ref_pop_array_bounds(:,p_upper.order);
mean_TE_wedge = mean(CM.ref_pop_array_bounds(:,p_upper.order));
mean_TE_wedge = mean(CM.ref_pop_array_bounds(:,p_upper.order));
CM.A_eq = zeros(size(CM.ref_pop_array_bounds(1,:)));
CM.A_eq(p_upper.order) = 1;
CM.b_eq = mean_TE_wedge;
