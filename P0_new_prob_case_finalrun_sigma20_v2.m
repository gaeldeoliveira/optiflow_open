...
case_overview.Re                 =     12e6  ;       % Reynolds Number (NaN means inviscid)
...
case_overview.N_cores            =  64   ;       % [int. ] - number of cores over which to spread computations
...
case_overview.PopulationSize     = 192   ;       % [int. ] - size of population in genetic pool
case_overview.Generations        =  60   ;       % [int. ] - number of generations to run genetic algorithm
...
case_overview.mingle_factor      = 0.25  ;       % [adim.] - Weighting between free and forced transition performance (0 = fully free, 1=fully tripped)
case_overview.n_sigma_gust       =   2.0 ;    % [adim.] - Number of standard deviations for extreme gust
...
%  ---- Edit any properties of choice, like number of cores ---- %
    ...
    SC.airfoil_subdir             = 'probabilistic/data_rnd5/airfoils/';
    SC.feasible_airfoil_subdir    = 'probabilistic/data_rnd5/feasible_airfoils/';
...
mingle_factor   = case_overview.mingle_factor;
n_sigma_gust    = case_overview.n_sigma_gust;
% Create expected glide ratio cost function
CF_LoverD = cost_function('max_expected_L_over_expectedD_with_roughness_resilience_constraint');    % Initialize it and give it a name
CF_LoverD.cost_function_handle = @cost_function_max_expected_CL_over_expected_CD_mingled_v2;        % Specify which function to use
CF_LoverD.post_function    = @(x) -x/100;                                                           % post_function, applied to result of cost_function_handle, for example to invert values or make negative
CF_LoverD.parameter_list   = {'n_sigma', 'n_experiment', 'bh_75_threshold' , 'bh_90_threshold' , 'sigma', 'mingle_factor', 'n_sigma_gust'};   % list of parameters names that control cost function execution
CF_LoverD.parameter_values = { n_sigma ,  n_free_exp   ,  bh_75_threshold  ,  bh_90_threshold  ,  sigma ,  mingle_factor ,  n_sigma_gust};   % list of parameters values that control cost function execution

% Create forced aerodynamics cost function
CF_CFexceedance = cost_function('cl_exceedance_in_gust');                                       % Initialize it and give it a name
CF_CFexceedance.cost_function_handle = @cost_function_max_expected_CL_exceedance;               % Specify which function to use
CF_CFexceedance.post_function    = @(x) x;                                                      % post_function, applied to result of cost_function_handle, for example to invert values or make negative
CF_CFexceedance.parameter_list   = {'n_sigma', 'n_experiment', 'bh_75_threshold' , 'bh_90_threshold' , 'sigma', 'mingle_factor', 'n_sigma_gust'};   % list of parameters names that control cost function execution
CF_CFexceedance.parameter_values = { n_sigma ,  n_free_exp   ,  bh_75_threshold  ,  bh_90_threshold  ,  sigma ,  mingle_factor ,  n_sigma_gust};   % list of parameters values that control cost function execution

%% Create the global cost function using the single objectives we defined earlier
% Use the global_cost_function class
CFG = global_cost_function('Free_expected_LD_at_AoA_VS_Cl_exceedance_in_gust');                 % Initialize it and give it a name
CFG.simulation_worker_list = {SW_1, SW_2};                                                      % Provide list of simulation workers to be executed on each cost function call
CFG.interpretation_function_list = {CF_LoverD , CF_CFexceedance};                               % Provide list of cost functions for interpretation of results
...
% Compute performance of pareto front designs
GM.call_cost_function(results.x); pareto_experiments = GM.CFG.last_experiment_set;
% Save results
...
GM.call_cost_function(GM.CM.ref_pop_array_bounds); inital_pop_experiments = GM.CFG.last_experiment_set;
save(['P20_v2_with_initial_population_for_reference', datestr(now, 30), '.mat']);
