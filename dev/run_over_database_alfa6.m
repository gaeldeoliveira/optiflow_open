...
SPR_fields = fieldnames(SPR_0);

% Find maximum index number
N_free_fields = 0;
for n_field = 1:length(SPR_fields)
    if SPR_free.(SPR_fields{n_field}) == 1
        N_free_fields = N_free_fields + 1;
    end
end

% Allocate parameter vectors
x_0  = zeros(1,N_free_fields);
x_lb = zeros(1,N_free_fields);
x_ub = zeros(1,N_free_fields);
...
for n_field = 1:length(SPR_fields)
    ...
    if SPR_free.(SPR_fields{n_field}) == 1
        ...
        i_parameter       = SPR_index.(SPR_fields{n_field});
        % Fill in initial point parameter vector
        x_0(i_parameter)  = SPR_0.(SPR_fields{n_field});
        % Fill in lower bounds parameter vector
        x_lb(i_parameter) = SPR_lb.(SPR_fields{n_field});
        % Fill in upper bounds parameter vector
        x_ub(i_parameter) = SPR_ub.(SPR_fields{n_field});
    end
end
