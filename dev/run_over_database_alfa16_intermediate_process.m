...
% Replace optimization run (going on right now on desktop, for run_over_database_alfa16.m)
% options = optimoptions(@simulannealbnd, ...
%                       'PlotFcn',{@saplotbestf,@saplottemperature,@saplotf,@saplotstopping});
% [x_sa,fval_sa,exitFlag_sa,output_sa] = simulannealbnd(min_fun_handle,GFW.x_0,GFW.x_lb,GFW.x_ub,options);
% With a curious case taken from the text logs of simulannealbnd:
x_sa = [1.0206        1.13     0.89913      1.0466      1.0859     0.92417     0.90867     0.94014];
%Save results
save([mfilename(),'_' datestr(now(), 30)])

% Compute population on results
[scalar_accuracy_metric, normalized_accuracy_profile, accuracy_profile, APE, SPR] = accuracy_profiles_from_parameters(GFW, x_sa);
