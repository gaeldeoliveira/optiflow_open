classdef shape_definition_cst < shape_meta_definition
    % SHAPE_DEFINITION This class stores, aquires and converts airfoil shape
    % definitions
     
    properties
        N_dummy_parameters  = 0;   % NUmber of parameters left at the end of the vector that are not related to the shape definition
        % a(end+1:end+N_dummy_parameters) = 0
    end
    ...
    methods
        ...
        function [parameters_upper , parameters_lower , z_te_upper , z_te_lower] = breakdown_parameters(SD , parameters)
...
%             parameters_lower = parameters(((N-1)/2+1):(N-1));
        ...
        function [tx_flapped , tz_flapped] = flap_airfoil_side(SD, tx, tz , flap_parameters)
            % This function is used to implement the flapping of airfoils
            % Puisque tu parts - Goldman
            cr_x = flap_parameters(1);          % Flap Rotation Center (chordwise coordinate)
            cr_z = flap_parameters(2);          % Flap Rotation Center (normal coordinate)
            phi  = flap_parameters(3);          % Flap Rotation Angle  (Positive is Clockwise, unlike for angle of attack)            
            
            % Rotation around arbitrary point is decomposed in 
            % translating to cr to origin -> rotating -> translating back
