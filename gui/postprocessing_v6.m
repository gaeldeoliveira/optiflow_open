...
function varargout = postprocessing_v6(varargin)
% POSTPROCESSING_V6 M-file for postprocessing_v6.fig
%      POSTPROCESSING_V6, by itself, creates a new POSTPROCESSING_V6 or raises the existing
%      singleton*.
%
%      H = POSTPROCESSING_V6 returns the handle to a new POSTPROCESSING_V6 or the handle to
%      the existing singleton*.
%
%      POSTPROCESSING_V6('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POSTPROCESSING_V6.M with the given input arguments.
%
%      POSTPROCESSING_V6('Property','Value',...) creates a new POSTPROCESSING_V6 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before postprocessing_v6_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to postprocessing_v6_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help postprocessing_v6

% Last Modified by GUIDE v2.5 13-Nov-2017 18:57:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @postprocessing_v6_OpeningFcn, ...
    'gui_OutputFcn',  @postprocessing_v6_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before postprocessing_v6 is made visible.
function postprocessing_v6_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to postprocessing_v6 (see VARARGIN)

% Choose default command line output for postprocessing_v6
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes postprocessing_v6 wait for user response (see UIRESUME)
% uiwait(handles.figure1);
end

% --- Outputs from this function are returned to the command line.
function varargout = postprocessing_v6_OutputFcn(hObject, eventdata, handles) %#ok<INUSL>
...
for n_plot = 1:length(plot_indices) % replace by for loop when debug complete
    ...
    if handles.checkbox3.Value == 0
        ...
        if ~isempty(result.ap.bldata)
            ...
            plot(handles.axesPareto, result.ap.bldata.xun_top_range, result.ap.bldata.cpx_fun_top(aoa_plot, result.ap.bldata.xun_top_range), 'Color', rainbow_list(n_plot, :)); 
            hold(handles.axesPareto, 'on'); grid(handles.axesPareto, 'on');
            plot(handles.axesPareto, result.ap.bldata.xun_bot_range, result.ap.bldata.cpx_fun_bot(aoa_plot, result.ap.bldata.xun_bot_range), 'Color', rainbow_list(n_plot, :));
