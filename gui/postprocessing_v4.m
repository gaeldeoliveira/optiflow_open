...
function varargout = postprocessing_v4(varargin)
% POSTPROCESSING_V4 M-file for postprocessing_v4.fig
%      POSTPROCESSING_V4, by itself, creates a new POSTPROCESSING_V4 or raises the existing
%      singleton*.
%
%      H = POSTPROCESSING_V4 returns the handle to a new POSTPROCESSING_V4 or the handle to
%      the existing singleton*.
%
%      POSTPROCESSING_V4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POSTPROCESSING_V4.M with the given input arguments.
%
%      POSTPROCESSING_V4('Property','Value',...) creates a new POSTPROCESSING_V4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before postprocessing_v4_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to postprocessing_v4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help postprocessing_v4

% Last Modified by GUIDE v2.5 13-Nov-2017 16:51:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @postprocessing_v4_OpeningFcn, ...
                   'gui_OutputFcn',  @postprocessing_v4_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before postprocessing_v4 is made visible.
function postprocessing_v4_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to postprocessing_v4 (see VARARGIN)

% Choose default command line output for postprocessing_v4
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes postprocessing_v4 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = postprocessing_v4_OutputFcn(hObject, eventdata, handles) 
...
for n_exclude = 1:length(exclude_indices)
    filtered_indices  = filtered_indices(find(not(filtered_indices == exclude_indices(n_exclude))));
end
...
for n_plot = 1:length(plot_indices) % replace by for loop when debug complete
    ...
    if ~isempty(result.ap.bldata)
        ...
        legend_list{n_plot} = [ 'th/c=' , num2str(thickness,3) , ' L/D_{max}=' , num2str(cl_over_cd_max,3), ' CP_min=' , num2str(cpmn, 3) , '@Cl=' , num2str(result.ap.cl_alpha(aoa_plot))];
    end                             
end
