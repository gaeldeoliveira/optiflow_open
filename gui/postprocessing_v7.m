...
function varargout = postprocessing_v7(varargin)
% POSTPROCESSING_V7 M-file for postprocessing_v7.fig
%      POSTPROCESSING_V7, by itself, creates a new POSTPROCESSING_V7 or raises the existing
%      singleton*.
%
%      H = POSTPROCESSING_V7 returns the handle to a new POSTPROCESSING_V7 or the handle to
%      the existing singleton*.
%
%      POSTPROCESSING_V7('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POSTPROCESSING_V7.M with the given input arguments.
%
%      POSTPROCESSING_V7('Property','Value',...) creates a new POSTPROCESSING_V7 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before postprocessing_v7_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to postprocessing_v7_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help postprocessing_v7

% Last Modified by GUIDE v2.5 13-Nov-2017 20:09:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @postprocessing_v7_OpeningFcn, ...
    'gui_OutputFcn',  @postprocessing_v7_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before postprocessing_v7 is made visible.
function postprocessing_v7_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to postprocessing_v7 (see VARARGIN)

% Choose default command line output for postprocessing_v7
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes postprocessing_v7 wait for user response (see UIRESUME)
% uiwait(handles.figure1);
end

% --- Outputs from this function are returned to the command line.
function varargout = postprocessing_v7_OutputFcn(hObject, eventdata, handles) %#ok<INUSL>
...
function uitable1_CellEditCallback(hObject, eventdata, handles) %#ok<INUSL>
...
if handles.checkbox3.Value == 0
    % First update pareto front (if applicable)
...
rainbow_list(1,:) = [0 0.4470 0.7410];
...
for n_plot = 1:length(plot_indices) % replace by for loop when debug complete
    ...
    cl_plot = sscanf(contents{handles.popupmenu1.Value}, '%f');
    ...
    if ~isempty(result.ap.bldata)
        ...
        cl_plot = sscanf(contents{handles.popupmenu1.Value}, '%f');
        % Get aoa
        aoa_plot = result.ap.alpha_cl(cl_plot);
        if isnan(aoa_plot)
        ...
        legend_list{n_plot} = [ 'th/c=' , num2str(thickness,3) , ' L/D_{max}=' , num2str(cl_over_cd_max,3), '    and     CP_{min}=' , num2str(cpmn, 3) , ' @Cl=' , num2str(result.ap.cl_alpha(aoa_plot))];
    end
    
    % Now plot on pareto axes
    if handles.checkbox3.Value == 0
        % Plot foils on pareto front if applicable
        hold(axes_parcp, 'on')
        plot(axes_parcp, handles.fval_sorted(plot_indices(n_plot),1), handles.fval_sorted(plot_indices(n_plot),2), 'o', 'Color', rainbow_list(n_plot, :))
    else
        % Otherwise try to plot pressure distribution
        if ~isempty(result.ap.bldata)
            % Get cl from popup
            contents = cellstr(handles.popupmenu1.String);
            cl_plot = sscanf(contents{handles.popupmenu1.Value}, '%f');
            ...
            plot(axes_parcp, result.ap.bldata.xun_top_range, result.ap.bldata.cpx_fun_top(aoa_plot, result.ap.bldata.xun_top_range), 'Color', rainbow_list(n_plot, :)); 
            hold(axes_parcp, 'on'); grid(axes_parcp, 'on');
            plot(axes_parcp, result.ap.bldata.xun_bot_range, result.ap.bldata.cpx_fun_bot(aoa_plot, result.ap.bldata.xun_bot_range), 'Color', rainbow_list(n_plot, :));
            xlabel(axes_parcp, 'x/c');ylabel(axes_parcp, 'C_p = Pressure Coefficient');
        else
            % Unless there is no bl data
            disp('Optiflow GUI: Pressure data unavailable')
        end
        
    end
end
% Reestablis no plotting for pareto axes
hold(axes_parcp, 'off');
...
title(axes_airfoil  , 'Airfoil Shapes')

axis(axes_clcd      , [0 0.03 -0.5 2]);
grid(axes_clcd      , 'on');
xlabel(axes_clcd    , 'C_{d}');
ylabel(axes_clcd    , 'C_{l}');
title(axes_clcd     , 'Drag Polars')

axis(axes_clalpha   , [-10 20 -0.5 2]);
grid(axes_clalpha   , 'on');
xlabel(axes_clalpha , '\alpha (deg)');
ylabel(axes_clalpha , 'C_{l}');
title(axes_clalpha  , 'Lift Polars')


title(axes_parcp    , ['Pressure Distribution at Cl=' , num2str(cl_plot)])
...
if for_print == true
    ...
    print(f14 , '-dpdf' , [handles.fname_root '_cp.pdf']);
end
...
function checkbox3_Callback(hObject, eventdata, handles) %#ok<DEFNU>
...
%handles.plot_pressure = get(hObject,'Value');
% And record it
%guidata(hObject, handles);
...
function popupmenu1_Callback(hObject, eventdata, handles) %#ok<INUSL,DEFNU>
