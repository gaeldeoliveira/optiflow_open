function AIRFOIL=calculate_structural_properties_airfoils(COORD,POINTS_OUTPUT)
% interpolate for 1000 points
COORD2=interpolate_1000_points(COORD,4000);
...
[XBEND,YBEND]=calculate_bending_parameters(COORD3);
AIRFOIL.XBEND=XBEND;
AIRFOIL.YBEND=YBEND;
...
function [XBEND,YBEND]=calculate_bending_parameters(COORD)
%%% assumes normalized and rotated coordinates LE=0,0 and TE=1,0
...
XBEND.SOLID_Iyy=sum(AVERAGE_HEIGHT_SLICES.*(-(XI(1:end-1)-XBEND.XC).^3+(XI(2:end)-XBEND.XC).^3))/3;
    DXL=COORD(2:end,1)-COORD(1:end-1,1);
    DYL=COORD(2:end,2)-COORD(1:end-1,2);
    DS=sqrt(DXL.^2+DYL.^2);
XBEND.Iyy_over_tsurf=sum(DS.*(((COORD(2:end,1)-XBEND.XC).^2+(COORD(1:end-1,1)-XBEND.XC).^2)/2));

%%%% calculate values of Ixx
YBEND.SOLID_Ixx=  sum(DX.* ...
    (((YUPPER(1:end-1)-YBEND.YC).^3+(YUPPER(2:end)-YBEND.YC).^3)/2 ...
    -((YLOWER(1:end-1)-YBEND.YC).^3+(YLOWER(2:end)-YBEND.YC).^3)/2)/3);

    DX=COORD(2:end,1)-COORD(1:end-1,1);
    DY=COORD(2:end,2)-COORD(1:end-1,2);
    DS=sqrt(DX.^2+DY.^2);
YBEND.Ixx_over_tsurf=sum(DS.*(((COORD(2:end,2)-YBEND.YC).^2+(COORD(1:end-1,2)-YBEND.YC).^2)/2));
end