function sc = cost_function_expected_CLCD_at_expected_AoA_w2(parameters , experiments_results)
...
%
%   With workaround for sigma60 (aka w2 for WORKAROUND 2)
...
try
    if strcmp(class(ap) , 'aerodynamic_polar') %#ok<STISA>
        ...
        % WORKAROUND 2: Make check to verify that we didn't pick the screwed drag branch
        ld_alfa_p3 = interp1(alpha_range, ld_alpha,  3);
        ld_alfa_m3 = interp1(alpha_range, ld_alpha, -3);
        if (ld_alfa_p3 > ld_alfa_m3)
            % Everything is normal, do nothing 
            % (this if-else use is resilient to NaNs)
        else
            % Entered strange drag branch for some reason
            sc = 0;
            lg.err(ap, ['Strange drag branch detected, x = ' , num2str(experiment_result.x)]);
        end
