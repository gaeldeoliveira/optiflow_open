function sc = cost_function_expected_CL_at_expected_AoA(parameters , experiments_results)
...
try
    if strcmp(class(ap) , 'aerodynamic_polar') %#ok<STISA>
        ...
        cl_alpha    = ap.cl_alpha(alpha_range);

        % Get expected L/D curve through convolution
        [alpha_bar_range, cl_expected] =   expected_polar_convolution(  sigma, n_sigma, n_alpha_til, alpha_range, cl_alpha);
        % Get expected L/D at max L/D point (from static case)
        cl_expected_at_alpha_bar = interp1(alpha_bar_range, cl_expected, alpha_bar);
        % Display
        disp(['Max CL  expt = ' , num2str(max(cl_expected)         , 4) , '       with sigma = ', num2str(sigma)                           ])
        disp(['Max CL  stat = ' , num2str(ap.cl_max_local_limited  , 4) , '     at alpha_max = ', num2str(ap.alpha_cl_max_local_limited, 3)]);
        disp(['Expected  CL = ' , num2str(cl_expected_at_alpha_bar , 4) , '     at alpha_bar = ', num2str(alpha_bar                    , 3)]);
        
        % Check that polar was converged enough
        if not(isnan(cl_expected_at_alpha_bar))
            % Convolution was succesful
            sc = cl_expected_at_alpha_bar;
        else
