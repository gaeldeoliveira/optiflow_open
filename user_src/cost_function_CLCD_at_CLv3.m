function sc = cost_function_CLCD_at_CLv3(parameters , experiments_results)
...
%% Fudge out points with too high mean glide ratio (L/D) (rough 200, smooth 250)
    if sc > 200
        sc = 0;
    end
%% Fudge out points with too high max glide ratio (L/D) (rough 200, smooth 250)
    if max(ap.cl_alpha(ap.alpha_range) ./ ap.cd_alpha(ap.alpha_range)) > 250
         sc = 0;
    end
    
%% Apply Penalty for Violation of Minimum Building Height
    % Extract Coordinates
