...
%     CQ_axi_RBM  = BR.CQ_axi_root_bending_moment;
    ...
    if BC_moded.bool_prob_coef == false
        ...
        [RES_axi, RES_tan, BD, BR] = bogo2_alfa7_BEMsolver(BC_moded); %#ok<ASGLU>
        % Duplicate as stat for CQ
        BC_moded_stat                = BC_moded;
        BC_moded_stat.bool_prob_coef = false;
        % Solve in stat mode for CQ
        [RES_axi_stat, RES_tan_stat, BD_stat, BR_stat] = bogo2_alfa7_BEMsolver(BC_moded_stat); %#ok<ASGLU>
        % Extract outputs
        CP          = BR.CP_rotor_segments              ;
        CQ_axi_RBM  = BR_stat.CQ_axi_root_bending_moment;
    end
end
