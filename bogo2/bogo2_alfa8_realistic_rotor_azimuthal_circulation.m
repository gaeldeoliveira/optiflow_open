...
a_axi     = BR.a_axi(    1,:);
a_tan     = BR.a_tan(    1,:);
mu_vector = BR.mu_vector(1,:);
Omega     = BR.Omega         ;
R         = BR.R             ;

% Induction factors
figure(1)
plot(mu_vector,  a_axi); hold on
plot(mu_vector,  a_tan); grid on
legend('a_{axi}', 'a_{tan}');
title('Axial and tangential induction factors')

% Azimuthal velocities
figure(2)
u_azi_movement = Omega * (mu_vector * R);
u_azi_rotating = Omega * (mu_vector * R) .* (1 + a_tan)                          ; % Inneficient explicit way to write this for equation etimology! (switch from rotating to inertial change)
u_azi_inertial = Omega * (mu_vector * R) .* (1 + a_tan) - Omega * (mu_vector * R); % Inneficient explicit way to write this for equation etimology! (switch from rotating to inertial change)
plot(mu_vector, u_azi_movement); hold on;
plot(mu_vector, u_azi_rotating); grid on;
plot(mu_vector, u_azi_inertial); 
legend('Rotating Frame - Movement', 'Rotating Frame - Movement + Induction', 'Inertial Frame - Flow (induction)');
title('Azimuthal flow velocities');

% Circulation
figure(3)
circulation_x = u_azi_inertial .* (2 .* pi .* mu_vector .* R);
plot(mu_vector, circulation_x); grid on

% Vg number
% Definition:
%   Vg = 1/Ue * (Gamma / (2 * pi * S)) * (L/delta)
% Where:
%   Ue      - Edge velocity -> assiciate with Uinf
%   Gamma   - Circulation   -> assiciate with circulation_x
%   S       - Half of distance between pairs of rotors -> associate with half of 5D
%   delta   - Boundary lauer thicknes associate with 2D
%   L       - Distance over which flow develops -> associate with distance from one rotor to the next 


D       = BC.R  * 2;     % Rotor diameter
nD      = 5        ;     % Distance between turbines (or turbine pairs) in diameters (consider square mesh of wind energy converters, with nD=5 spacing)
S       = nD* D / 2;     % Half of distance between pairs of rotors (to the side)
L       = nD* D    ;     % Distance from one rotor to the next (downstream) 
deltaBL = 2 * D    ;
Ue      = BC.U_inf ;

Vg      = 1/Ue * (abs(circulation_x) / (2 * pi * S)) * (L/deltaBL);
plot(mu_vector, Vg)
